...
const { inferPurposeCode, purposeCodes } = require("./purposeCodes");
...
function extractPaymentDetails(text) {
  ...
  return {
    ...
    beneficiaryAccountNumber,
    beneficiaryName,
    purposeCode: inferPurposeCode(cleaned),
...
function buildOpenAiPrompt(localExtracted, missingFields) {
  ...
  const purposeCodeBlock = `Valid Singapore GIRO purpose codes:\n${purposeCodes
    .map((entry) => `${entry.purpose_code}: ${entry.description}`)
    .join("\n")}`;
  ...
  return [
    ...
    "For purposeCode, choose exactly one valid 4-character Singapore GIRO purpose code from the list. Use SUPP for normal supplier payments unless the document clearly supports a more specific code such as IVPT, GDDS, SCVE, TAXS, GSTX, UBIL, or SALA.",
    purposeCodeBlock,
