...
function normalizeBankSwiftCode(value = "") {
  const cleaned = sanitizeBankDetails(value).toUpperCase();
...
function sanitizeBeneficiaryName(value = "") {
  return String(value).replace(/[^A-Za-z0-9 ]/g, "");
}

function resolveHeaderForCurrency(currency = "") {
  const normalizedCurrency = String(currency || "").trim().toUpperCase();

  if (normalizedCurrency === "USD") {
    return {
      ...bankConfig.header,
      payerAccountNumber: "601425952201",
    };
  }

  return { ...bankConfig.header };
}

function createHeaderRow(overrides = {}) {
  const header = {
    ...resolveHeaderForCurrency(overrides.currency),
...
function createTransactionRow(paymentRecord) {
  ...
  line = writeField(
    ...
    `${normalizeBankSwiftCode(merged.bankSwiftCode)}${sanitizeBankDetails(merged.beneficiaryAccountNumber)}`
  );
  line = writeField(
    line,
    layout.beneficiaryNameStart,
    layout.beneficiaryNameLength,
    sanitizeBeneficiaryName(merged.beneficiaryName)
  );
  line = writeField(
    line,
    layout.amountStart,
  ...
  line = writeField(
    ...
    sanitizeAlphanumeric(merged.purposeCode || merged.remark)
  );
