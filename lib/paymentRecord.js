...
const { normalizePurposeCode } = require("./purposeCodes");
...
function normalizePurposeCodeInput(value) {
  return normalizeExportAlphanumeric(value).toUpperCase();
}
...
function buildPaymentRecord(input = {}) {
  ...
  const merged = {
    ...
    purposeCode: normalizePurposeCodeInput(
      corrected.purposeCode || extracted.purposeCode || input.purposeCode
    ),
  ...
  if (!merged.purposeCode) {
    merged.purposeCode = "SUPP";
  }
...
function validatePaymentRecord(paymentRecord) {
  ...
  if (merged.purposeCode && !normalizePurposeCode(merged.purposeCode)) {
    missingFields.push("purposeCode");
  }
